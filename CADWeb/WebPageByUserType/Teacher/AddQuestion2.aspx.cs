...
namespace WebApplication2
{
    public partial class AddQuestion2 : System.Web.UI.Page
    {
        ...
        protected void AdQuestion_Click(object sender, EventArgs e)
        {
            ...
            if (src == null)
            {
                sql = "INSERT INTO 判断题库 (题目,答案,答案解析,题目状态) VALUES ('" + question + "','" + answer
                + "','" + answerjx + "',1)";
            }
            else
            {
                sql = "INSERT INTO 判断题库 (题目,答案,答案解析,图片路径,题目状态) VALUES ('" + question + "','" + answer
                   + "','" + answerjx + "','" + src + "',1)";
            }
        ...
        protected void judgeToSql_Click(object sender, EventArgs e)
        {
        ...
        public void Bind(string fileName, string classtable)//获取excel中的数据
        {
            ...
            OleDbDataAdapter da = new OleDbDataAdapter("SELECT *  FROM [判断题$]", strConn);//选中excel工作表
        ...
        private bool InsertToSql(System.Data.DataTable table, string classtable)//导入数据库
        {
            ...
            try
            {
                using (SqlConnection conn = SQLConnect.GetConnection())
                {
                    ...
                    {
                        bulkCopy.DestinationTableName = "dbo.判断题库";//设置数据库中对象的表名
